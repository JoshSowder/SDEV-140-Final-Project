...
class PCT:
    def __init__(self):
        ...
        self.searchBt = Button(frame, text = "Search", command = self.displayItems(self.webscraper(self.v1.get()))).grid(row = 1, column = 16, pady = 10, ipadx = 4, ipady = 2)
        self.display = Listbox(frame, width = 175, height = 50).grid(row = 2, column = 1, rowspan = 25, columnspan = 25, padx = 10, pady = 10)
        ...
        self.comparator = Listbox(frame, width = 60, height = 50).grid(row = 2, column = 26, rowspan = 25, columnspan = 8, padx = 10, pady = 10)

        window.mainloop()
       
    def webscraper(self, searchitem):
        count = 0
        ...
        while count <= 3:
            if count == 1:
                driver.get('<a href="https://www.amazon.com/s?k="' + searchitem + '</a>')
                content = driver.page_source
                soup = BeautifulSoup(content)
                for a in soup.findAll(a, href = True, attrs = {'class':'s-result-item s-asin sg-col-0-of-12 sg-col-16-of-20 AdHolder sg-col s-widget-spacing-small sg-col-12-of-16'}):
                ...
                self.df1 = pd.DataFrame({'Product Name':products, 'Price':prices, 'Rating':ratings}) 
                self.df1.to_csv('products.csv', index=False, encoding='utf-8') #For testing and debugging
            elif count == 2:
                driver.get('<a href="https://www.walmart.com/search?q="' + searchitem + '</a>')
                content = driver.page_source
                soup = BeautifulSoup(content)
                for a in soup.findAll(searchitem, href = True, attrs = {'class':'mb1 ph1 pa0-xl bb b--near-white w-25'}):
                    ...
                    prices.append(price.text)
                    ratings.append(rating.text)
                self.df2 = pd.DataFrame({'Product Name':products, 'Price':prices, 'Rating':ratings}) 
                self.df2.to_csv('products.csv', index=False, encoding='utf-8') #For testing and debugging
            else:
                driver.get('<a href="https://www.target.com/s?searchTerm="' + searchitem + '</a>')
                content = driver.page_source
                soup = BeautifulSoup(content)
                for a in soup.findAll(searchitem, href = True, attrs = {'class':'_31qSD5'}):
                    ...
                    prices.append(price.text)
                    ratings.append(rating.text)
                self.df3 = pd.DataFrame({'Product Name':products, 'Price':prices, 'Rating':ratings}) 
                self.df3.to_csv('products.csv', index=False, encoding='utf-8') #For testing and debugging
            count += 1
